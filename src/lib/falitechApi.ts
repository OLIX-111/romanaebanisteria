...
export async function createSubscriber(payload: {
  ...
  export async function getSubscriberInfo(user_ns: string) {
    const url = `${process.env.FALITECH_API_URL}/subscriber/get-info?user_ns=${encodeURIComponent(user_ns)}`
    const res = await fetch(url, {
      headers: {
        Authorization: `Bearer ${process.env.FALITECH_API_KEY}`
      }
    ...
    if (!res.ok) {
      const errorBody = await res.json().catch(() => ({}))
      throw new Error(`getSubscriberInfo failed: ${res.status} - ${JSON.stringify(errorBody)}`)
    }
    return res.json()
  }
  ...
  export async function createOrder(payload: {
    ...
    const url = `${process.env.FALITECH_API_URL}/shop/orders/create`
    const res = await fetch(url, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${process.env.FALITECH_API_KEY}`,
    ...
    if (!res.ok) {
      const errorBody = await res.json().catch(() => ({}))
      throw new Error(`createOrder failed: ${res.status} - ${JSON.stringify(errorBody)}`)
    }
    return res.json()
  }
