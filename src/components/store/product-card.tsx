...
export function ProductCard({ id, name, image, price, description, type, vendor, currency = "DOP", onQuickView }: ProductCardProps) {
  ...
  return (
    <Link href={`/store/${id}`} className="group relative flex flex-col border border-gray-200 bg-white transition">
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        <div className="absolute left-2 top-2 flex flex-wrap gap-1">
          <span className="bg-white/80 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-gray-700 backdrop-blur">{type}</span>
        </div>
        {onQuickView && (
          <button
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-3 flex items-end justify-between gap-2">
          ...
          <span className="bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-600">{vendor}</span>
        </div>
      </div>
    </Link>
  )
